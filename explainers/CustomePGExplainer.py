...
# -*- coding: utf-8 -*-
"""
Created on Fri May 12 16:19:02 2023
...
class PGExplainerCustome(ExplainerAlgorithm):
    ...
    def train(
        ...
        **kwargs,
        ...
        # print(edge_index[0])
        # print(edge_index[1])        
        z = get_embeddings(model, data, data.edge_index, **kwargs)[-1]
        ...
        #     model.edge_index1 = torch.stack([row, row], dim=0)
        #     # edge_index = edge_index1
        #     # z = (1,45) to (32,45)
        #     z = z.repeat(row.size(0),1)
        #     # edge_index = edge_index1
        # # else:
        ...
        y_hat, y = model(data, edge_index, **kwargs), target
    ...
    def forward(
        ...
        **kwargs,
        ...
        if isinstance(data.x, dict) or isinstance(edge_index, dict):
            raise ValueError(f"Heterogeneous graphs not yet supported in "
                             f"'{self.__class__.__name__}'")
        ...
        # z = get_embeddings(model, x, edge_index, **kwargs)[-1]
        z = get_embeddings(model, data, **kwargs)[-1]
